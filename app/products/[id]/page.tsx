...
export default function ProductId({
  ...
  return (
    <div className="m-10 flex gap-3 border border-slate-600 max-w-3xl p-3 bg-slate-700">
      ...
      <div className="flex flex-col">
        ...
        <ul>
          {product.sizes.map((size) => (
            <li key={size.sku}>
              {size.available ? <p className="text-slate-200">{size.size}</p> : <p className="text-slate-400/70">{size.size}</p>}
            </li>
          ))}
        </ul>
